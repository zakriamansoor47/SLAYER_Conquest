...
public partial class SLAYER_Conquest : BasePlugin, IPluginConfig<SLAYER_ConquestConfig>
{
    ...
    {
        if (player == null || !player.IsValid || reviver == null || !reviver.IsValid || player.PlayerPawn.Value!.LifeState == (byte)LifeState_t.LIFE_ALIVE || GetPlayerReviveEntry(player) == null || player.TeamNum != reviver.TeamNum || IsPlayerGettingRevived(player)) return null;
    ...
    {
        if (player == null || !player.IsValid || !IsPlayerReviving(player) || AvailableRevives == null) return null;

        var reviveEntry = AvailableRevives.FirstOrDefault(revive => revive.reviver != null && revive.reviver.IsValid && revive.reviver == player && revive.status == ReviveStatus.Reviving);
        if (reviveEntry == null || reviveEntry.player == null || !reviveEntry.player.IsValid) return null;
        return reviveEntry.player;
    }
    private void StartReviving(CCSPlayerController? player, CCSPlayerController? reviver, ReviverType? reviveType)
    {
        if (player == null || !player.IsValid || reviver == null || !reviver.IsValid || reviveType == null) return;
    ...
    private void UpdateReviveStatus()
    {
        ...
        foreach (var reviveEntry in reviveEntriesToProcess)
        {
            ...
            if (reviveEntry.status == ReviveStatus.Reviving)
            {
                ...
                if (reviveEntry.reviver == null || reviveEntry.player == null || !reviveEntry.reviver.IsValid || reviveEntry.reviver.Connected != PlayerConnectedState.PlayerConnected || reviveEntry.reviver.TeamNum != reviveEntry.player.TeamNum || reviveEntry.reviver.PlayerPawn.Value!.LifeState != (byte)LifeState_t.LIFE_ALIVE)
                {
                ...
                else if (reviveEntry.player != null && reviveEntry.player.IsValid && reviveEntry.player.TeamNum != reviveEntry.PlayerTeamNum || reviveEntry.player!.PlayerPawn.Value!.LifeState == (byte)LifeState_t.LIFE_ALIVE)
                {
    ...
    {
        if (player == null || !player.IsValid || player.Connected != PlayerConnectedState.PlayerConnected || player.PlayerPawn.Value!.LifeState != (byte)LifeState_t.LIFE_ALIVE) return false;
    ...
    {
        if (player == null || !player.IsValid || player.Connected != PlayerConnectedState.PlayerConnected || player.PlayerPawn.Value!.LifeState == (byte)LifeState_t.LIFE_ALIVE) return;
        ...
        {
            foreach (var medic in FindNearbyMedicsOrSquadmates(player).Where(m => m != null && m.IsValid && m.Connected == PlayerConnectedState.PlayerConnected && m.TeamNum == player.TeamNum && m.PlayerPawn.Value!.LifeState == (byte)LifeState_t.LIFE_ALIVE))
            {
    ...
    private CCSPlayerController? FindNearestTeammate(CCSPlayerController? player, float maxDistance = 100f, bool DeadOnly = false)
    {
        ...
        var nearestTeammate = Utilities.GetPlayers()
            .Where(p => p != null && p.IsValid && p.Connected == PlayerConnectedState.PlayerConnected && !p.IsHLTV && p.TeamNum > 1  && p != player && (DeadOnly ? p.PlayerPawn.Value!.LifeState != (byte)LifeState_t.LIFE_ALIVE : p.PlayerPawn.Value!.LifeState == (byte)LifeState_t.LIFE_ALIVE) && !IsPlayerGettingRevived(p)) // Not alive and not currently getting revived
            .Select(entry => new
            {
                Player = entry,
        ...
        if (nearestTeammate?.Player == null || !nearestTeammate.Player.IsValid || nearestTeammate.Player.Connected != PlayerConnectedState.PlayerConnected || (DeadOnly ? nearestTeammate.Player.PlayerPawn.Value!.LifeState == (byte)LifeState_t.LIFE_ALIVE : nearestTeammate.Player.PlayerPawn.Value!.LifeState != (byte)LifeState_t.LIFE_ALIVE))
        {
            return null;
        }
    ...
    private List<CCSPlayerController> FindNearbyMedicsOrSquadmates(CCSPlayerController? player)
    {
        ...
        var nearbyMedics = Utilities.GetPlayers()
            .Where(p => p != null && p.IsValid && p.Connected == PlayerConnectedState.PlayerConnected && !p.IsHLTV && p.TeamNum == player.TeamNum && p.PlayerPawn.Value!.LifeState == (byte)LifeState_t.LIFE_ALIVE && p != player &&
