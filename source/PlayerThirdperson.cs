...
public partial class SLAYER_Conquest : BasePlugin, IPluginConfig<SLAYER_ConquestConfig>
{
    ...
    public HookResult OnWeaponZoom(EventWeaponZoom @event, GameEventInfo info)
    {
        ...
        if (player == null || !player.IsValid || player.Connected != PlayerConnectedState.PlayerConnected || player.IsBot || player.IsHLTV || player.PlayerPawn.Value!.LifeState != (byte)LifeState_t.LIFE_ALIVE || player.TeamNum < 2)
            return HookResult.Continue;

        Server.NextFrame(() =>
        {
            if (ThirdPerson != null && ThirdPerson.ContainsKey(player))
            {
    ...
    {
        if (player == null || !player.IsValid || player.PlayerPawn == null || !player.PlayerPawn.IsValid || player.PlayerPawn.Value == null || player.Connected != PlayerConnectedState.PlayerConnected || player.PlayerPawn.Value!.LifeState != (byte)LifeState_t.LIFE_ALIVE || player.TeamNum < 2) return null;
    ...
    public void UpdateCameraSmooth(CPhysicsPropMultiplayer cameraProp, CCSPlayerController player)
    {
        ...
        Vector playerEyePos = GetEyePosition(player);
    ...
    private Vector GetWallSafeCameraPosition(CCSPlayerController player, Vector desiredCameraPos, Vector playerEyePos)
    {
        ...
        TraceOptions options = new TraceOptions(InteractionLayers.MASK_SHOT_FULL, (InteractionLayers)player.PlayerPawn.Value.Collision.CollisionAttribute.InteractsWith, (InteractionLayers)player.PlayerPawn.Value.Collision.CollisionAttribute.InteractsExclude);
        ...
        TraceShape(playerEyePos, desiredCameraPos, player.PlayerPawn.Value, options, out var mainTrace);

        if (mainTrace.DidHit)
        {
            Vector hitPoint = ConvertVector3ToVector(mainTrace.HitPoint);
            ...
            if (distanceToWall < desiredDistance)
            {
                ...
                foreach (float forwardDist in forwardDistances)
                {
                    ...
                    if (IsPositionSafe(player, forwardPos, playerEyePos, options))
                    {
                ...
                foreach (var altDirection in alternativeDirections)
                {
                    ...
                    foreach (float altDist in altDistances)
                    {
                        ...
                        if (IsPositionSafe(player, altPos, playerEyePos, options))
                        {
                ...
                if (IsPositionSafe(player, lastResortPos, playerEyePos, options))
                {
    ...
    /// </summary>
    private bool IsPositionSafe(CCSPlayerController player, Vector cameraPos, Vector playerEyePos, TraceOptions options)
    {
        ...
        foreach (var offset in testOffsets)
        {
            ...
            TraceShape(playerEyePos, testPos, player.PlayerPawn.Value, options, out var trace);

            if (trace.DidHit)
            {
                Vector hitPoint = ConvertVector3ToVector(trace.HitPoint);
