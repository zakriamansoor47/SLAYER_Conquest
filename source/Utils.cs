...
public partial class SLAYER_Conquest : BasePlugin, IPluginConfig<SLAYER_ConquestConfig>
{
    ...
    {
        if(client == null || !client.IsValid || client.Connected != PlayerConnectedState.PlayerConnected || client.PlayerPawn.Value!.LifeState != (byte)LifeState_t.LIFE_ALIVE)return;
    ...
    public CEnvParticleGlow? ParticleCreate(string model, Vector startPos, QAngle rotation, float duration = -1, Color? color = null, float scale = 1.0f, float RadiusScale = 200f, float AlphaScale = 1f, CBaseEntity? parent = null)
    {
        ...
        particle.RenderMode = RenderMode_t.kRenderTransAlpha;
    ...
    {
        if (player == null || !player.IsValid || player.PlayerPawn.Value!.LifeState != (byte)LifeState_t.LIFE_ALIVE || player.PlayerPawn.Value.MoveType == MoveType_t.MOVETYPE_NONE) return;
    ...
    {
        if (player == null || !player.IsValid || player.PlayerPawn.Value!.LifeState != (byte)LifeState_t.LIFE_ALIVE || player.PlayerPawn.Value.MoveType == MoveType_t.MOVETYPE_WALK) return;
    ...
    public static void ChangePlayerTeam(CCSPlayerController? client, CsTeam team)
    {
        ...
        Server.NextFrame(() =>
        {
            ...
            if (client.PlayerPawn.Value!.LifeState == (byte)LifeState_t.LIFE_ALIVE && team == CsTeam.Terrorist)
            {
                client.PlayerPawn.Value.SetModel("characters/models/tm_phoenix/tm_phoenix.vmdl");
            }
            else if (client.PlayerPawn.Value!.LifeState == (byte)LifeState_t.LIFE_ALIVE && team == CsTeam.CounterTerrorist)
            {
    ...
    public Vector? GetPlayerAimPosition(CCSPlayerController? player)
    {
        ...
        return ConvertVector3ToVector(trace.EndPos);
    }
    public bool IsPlayerStuck(CCSPlayerController player)
    {
        if (player == null || !player.IsValid || player.PlayerPawn.Value == null || player.PlayerPawn.Value.LifeState != (byte)LifeState_t.LIFE_ALIVE) return false;
    ...
    public bool IsPlayerDetected(Vector _origin, Vector _endOrigin, TraceOptions options, CCSPlayerPawn? pawn = null)
    {
        TraceShape(_origin, _endOrigin, pawn, options, out var trace);
    ...
    private bool IsPlayerInLOS(CCSPlayerController player1, CCSPlayerController player2)
    {
        ...
        return IsPlayerDetected(GetEyePosition(player1), GetEyePosition(player2), new TraceOptions((InteractionLayers)player1.PlayerPawn!.Value!.Collision.CollisionAttribute.InteractsAs, (InteractionLayers)player1.PlayerPawn!.Value!.Collision.CollisionAttribute.InteractsWith, (InteractionLayers)player1.PlayerPawn.Value!.Collision.CollisionAttribute.InteractsExclude), player1.PlayerPawn.Value);
    }
    ...
    private List<CBeam> DrawBeaconCircle(Vector position, float circle_radius, int TotalBeams, Color color, float beamWidth = 2f)
    {
        ...
        for (int i = 0; i < lines; i++) // Drawing Beacon Circle
        {
            ...
            var beam = DrawLaserBetween(start, end, color, -1, beamWidth);
            if (beam != null) beam_ent.Add(beam);
