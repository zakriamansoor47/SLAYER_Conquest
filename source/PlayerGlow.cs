...
public partial class SLAYER_Conquest : BasePlugin, IPluginConfig<SLAYER_ConquestConfig>
{
    ...
    public void SetGlowOnSquadMembers(CCSPlayerController player)
    {
        ...
        if (squad != null)
        {
            foreach (var member in squad.Members.Keys.Where(m => m != null && m.IsValid && m.Connected == PlayerConnectedState.PlayerConnected && m.TeamNum == player.TeamNum && m != player && m.PlayerPawn.Value != null))
            {
                ...
                if (IsPlayerInLOS(player, member) && member.PlayerPawn.Value!.LifeState == (byte)LifeState_t.LIFE_ALIVE)
                {
    ...
    public void SetGlowOnMedic(CCSPlayerController player)
    {
        ...
        {
            if (!IsPlayerBehind(player, medic) && player.PlayerPawn.Value!.LifeState != (byte)LifeState_t.LIFE_ALIVE)
            {
