...
public partial class SLAYER_CaptureTheFlag : BasePlugin, IPluginConfig<SLAYER_CaptureTheFlagConfig>
{
    ...
    private bool UseMedicPouch(CCSPlayerController player)
    {
        ...
        foreach (var target in Utilities.GetPlayers().Where(p => p != null && p.IsValid && p.Connected == PlayerConnectedState.PlayerConnected && !p.IsHLTV && p.TeamNum == player.TeamNum && p.Pawn.Value!.LifeState == (byte)LifeState_t.LIFE_ALIVE && p != player))
        {
            ...
            if (distance <= Config.SpecialItems["MedicPouch"].Range && !IsPlayerBehind(player, target) && IsPlayerDetected(new Vector(playerPos.X, playerPos.Y, playerPos.Z + 15f), new Vector(targetPos.X, targetPos.Y, targetPos.Z + 15f), player.PlayerPawn!.Value!.Collision.CollisionAttribute.InteractsWith, player.PlayerPawn.Value!.Collision.CollisionGroup, player)) // 200 unit radius
            {
    ...
    private bool UseAmmoPouch(CCSPlayerController player)
    {
        ...
        foreach (var target in Utilities.GetPlayers().Where(p => p != null && p.IsValid && p.Connected == PlayerConnectedState.PlayerConnected && !p.IsHLTV && p.Pawn.Value!.LifeState == (byte)LifeState_t.LIFE_ALIVE && p.TeamNum == player.TeamNum && p != player))
        {
            ...
            float distance = CalculateDistanceBetween(playerPos, targetPos);
            if (distance <= Config.SpecialItems["AmmoPouch"].Range && !IsPlayerBehind(player, target) && IsPlayerDetected(new Vector(playerPos.X, playerPos.Y, playerPos.Z + 15f), new Vector(targetPos.X, targetPos.Y, targetPos.Z + 15f), player.PlayerPawn!.Value!.Collision.CollisionAttribute.InteractsWith, player.PlayerPawn.Value!.Collision.CollisionGroup, player)) // 200 unit radius
            {
    ...
    private bool ThrowPouch(CCSPlayerController thrower, CCSPlayerController target, bool isMedicPouch = false)
    {
        ...
        {
            if (tempDeployedItem == null || !tempDeployedItem.IsValid || tempDeployedItem.Entity == null || !tempDeployedItem.Entity.IsValid)
            {
    ...
    private void HealPlayer(CCSPlayerController player, int healAmount)
    {
        ...
        {
            if (player == null || !player.IsValid || player.PlayerPawn == null || !player.PlayerPawn.IsValid || player.Pawn.Value!.LifeState != (byte)LifeState_t.LIFE_ALIVE)
            {
    ...
    private void CheckClaymoreProximity(CCSPlayerController deployer, PlayerSpecificItems claymoreItem, DeployedItemInfo deployedItem, bool forceExplode = false)
    {
        ...
        foreach (var player in Utilities.GetPlayers().Where(p => p != null && p.IsValid && p.Connected == PlayerConnectedState.PlayerConnected && !p.IsHLTV && p.TeamNum > 1 && p.Pawn.Value!.LifeState == (byte)LifeState_t.LIFE_ALIVE))
        {
            ...
            if (distance <= Config.SpecialItems["Claymore"].Range && player.TeamNum != deployer.TeamNum) // trigger radius (only enemies)
            {
                ...
                var isDetected = IsPlayerDetected(pos, playerPos, player.PlayerPawn!.Value!.Collision.CollisionAttribute.InteractsWith, player.PlayerPawn.Value!.Collision.CollisionGroup, deployer); // only trigger if player is detected infront of claymore, not behind walls
